...
module.exports = class ValheimLauncher {
    ...
    async startValheim() {
        ...
        if (running) throw new Error('A valheim dedicated server is already running \n' + running);

        // Launch the server
        this.vdsProcess = spawn(this.launchFile, this.spawnOptions);
        this.vdsProcess.stdout.on('data', data => {
            data = data.toString().replace('\n', '');
            this.serverLog.detail(data);
            this.recentLogs.push(data);
        });
        this.vdsProcess.stderr.on('data',  data => {
            data = data.toString().replace('\n', '');
            this.serverLog.error(data, false);
            this.recentLogs.push(data);
        });
